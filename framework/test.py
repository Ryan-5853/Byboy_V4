...
任务：
1. 先对入口网页执行链接提取：{entry_url}
2. 再读取同一个入口网页正文：{entry_url}
3. 只根据这两个工具结果，总结该页面关于 OpenAI / OpenAI-compatible 模型配置的要点

要求：
- 必须只调用两次工具：第一次 `web_extract_links`，第二次 `web_fetch_url`
- 你需要在 memory_trace 中按顺序记录你每一步看到了什么、为什么下一步这样做
- 如果某个工具返回 TOOL_ERROR，不要继续尝试其他 URL；直接记录失败原因并给出已知信息
...
def preflight_model_backend() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Make sure the model service is listening at framework/llm_select/models.yaml base_url."
        ) from exc
