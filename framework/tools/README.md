...
渲染页面并抓取 XHR/fetch 请求，同时保存会话上下文（cookie/storage state）并返回 `session_id`。
...
```json
{"url":"https://homepage.hit.edu.cn/baixu?lang=zh","wait_until":"domcontentloaded"}
```

返回包含：`session_id`、`requests`、`captured_count`，可直接用于后续 `browser.replay_api`。

### `browser.replay_api`

在 `browser.capture_xhr` 返回的同一浏览器会话里重放 API 请求，自动携带会话状态。
...
tools:
  - name: browser.replay_api
    config:
      backend: playwright
      timeout_seconds: 30
      max_response_body_chars: 120000
      allowed_schemes: [http, https]
      allowed_domains:
        - "hit.edu.cn"
        - "*.hit.edu.cn"
...
```json
{
  "session_id":"<capture_xhr返回值>",
  "url":"https://homepage.hit.edu.cn/TeacherHome/teacherBody.do",
  "method":"POST",
  "data":{"id":"1904","versionId":""},
  "headers":{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"}
}
```

适用场景：`web.post_url` 在无会话条件下失败（如 `sql error`、空响应）时，用会话重放方式取动态接口内容。

### `browser.export_session`

将内存中的浏览器会话（cookie/storage_state）导出到 workspace JSON 文件，便于人工过盾后复用。
...
```json
{"session_id":"<capture_xhr返回>", "path":"workspace/某学院/session_state/ustc.json"}
```

### `browser.import_session`

从 workspace JSON 文件导入会话，返回新的 `session_id`（可用于 `browser.replay_api`）。
...
```json
{"path":"workspace/某学院/session_state/ustc.json"}
```
...
- 当最近出现相似查询且都为空时，提示会强调“连续相似空结果”，帮助模型收敛而不是循环。
...
```json
{"path": "framework/agent_router/README.md"}
```
...
```json
{"command": "pdftotext workflow/User/resume.pdf -"}
```
...
请遵循 [framework/tool_call/README.md](framework/tool_call/README.md) 中的 `ToolSpec` 接口规范。新工具建议按类别放入子包，例如：
